...
Интересно, что последний цикл не [векторизуется](sse.html) компилятором автоматически (видимо, потому что `std::min` это что-то сложное внутри), и поэтому замена тела цикла на конструкцию вида `(x < y ? x : y)` ускоряет построение ещё в ~2 раза.
...
Разреженную таблицу часто применяют для решения задачи о [наименьшем общем предке](lca.html), так как её можно свести к RMQ.
...
Это работает быстрее, чем, например, [дерево отрезков](st.html), но тоже асимптотически за $\O(\log n)$, да ещё и с дополнительной памятью. Но есть способ это ускорить.
...
```cpp
// Реализация не проверялась на баги.
