...
Поскольку у прямых хешей сильно криво идет поиск подстроки, то в приведенном коде используются _обратные_ хеши.

```cpp
const int P = 1000003;  // Для малых алфавитов можно брать P меньше, к примеру 179

// Структура, реализующая хеши по нескольким модулям,
// в данном случае используются 4 простых модуля 1e9+7, 1e9+9, 1e9+21, 1e9+33
const vector<int> M = {1000000007, 1000000009, 1000000021, 1000000033};
struct multihash {
  vector<int> vals;
  ...
  multihash(int val = 0) {
    ...
    for (int i = 0; i < M.size(); ++i)
      vals[i] = (val % M[i] + M[i]) % M[i];
  }

// прошу вас, НИКОГДА НЕ ПИШИТЕ ТАК
// лучше копи-паст чем дефайны
#define DefineOperation(x)                                                   \
  multihash operator x(const multihash& o) const {                           \
    multihash r;                                                             \
    for (int i = 0; i < M.size(); ++i)                                       \
      r.vals[i] =                                                            \
          (((long long)vals[i] x(long long)o.vals[i]) % M[i] + M[i]) % M[i]; \
    return r;                                                                \
  }
  ...
  DefineOperation(-);
  DefineOperation(*);
#undef DefineOperation(x)

  bool operator==(const multihash& o) {
    for (int i = 0; i < M.size(); ++i)
      if (vals[i] != o.vals[i])
        return false;
    return true;
...
vector<multihash> pow_P;

// слияние хешей
multihash addhash(multihash h1, multihash h2, int len2) {
  return h1 * pow_P[len2] + h2;
}

// хеш подстроки [le; ri]
multihash subhash(const vector<multihash>& hash, int le, int ri) {
  return hash[ri + 1] - hash[le] * pow_P[ri - le + 1];
}

// по строке вернуть все префхеши
vector<multihash> tohash(const vector<int>& s) {
  vector<multihash> h(s.size() + 1);
  for (int i = 1; i <= s.size(); ++i)
    h[i] = addhash(h[i - 1], s[i - 1], 1);
  return h;
}

// до вызова слияния хешей или получения хеша подстроки
// надо инициализировать степени P
void init_pow_p(int x) {
  pow_P.assign(x + 1, 1);
  for (int i = 0; i < x; ++i)
    pow_P[i + 1] = pow_P[i] * P;
}
...
TODO перетащить в [теорвер](prob.html)
