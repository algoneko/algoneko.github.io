...
Существует также и альтернативный способ получения итераторов начала и конца --- `begin(cont)` и `end(cont)`. Этот метод позволяет получать псевдо-итераторы для C-style массивов константного размера.
