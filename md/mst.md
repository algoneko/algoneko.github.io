...
Наиболее полный вариант кода DSU, который поддерживает выбор эвристик и сохранение допданных.
...
private:
  ...
  bool way_opt;  // Сжатие путей 0:отключить, 1:включить
  int rank_opt;  // Ранговая эвристика 0:нет, 1:глубина, 2:число вершин
...
public:
  ...
  void reserve(int x) { d.reserve(x); }
  void resize(int new_size) { d.resize(new_size); }
...
private:
  // Unsafe begin
  inline int& d_parent(int v) { return d[v].parent; }
  inline int& d_depth(int v) { return d[v].depth; }
  // Unsafe end
  ...
  int find_set_impl(int v) {
    ...
    // Если используется эвристика сжатия путей, то переопределяем предка
    if (way_opt)
      d_parent(v) = ancestor;
    return ancestor;
  }
  ...
  bool union_sets_impl(int a, int b) {
    ...
    b = find_set_impl(b);
    if (a != b) {
      // Выставить нового предка наименьшему дереву.
      // При отключенной ранговой эвристике, подвешиваем правое поддерево к левому
...
- Алгоритм Крускала — частный случай [алгоритма Радо-Эдмондса](matroid.html).
...
О решении этой задачи в online и в offline можете почитать [тут](dcp.html).
