...
Немногие знают, что существует функция для нахождения гипотенузы прямоугольного треугольника с заданными катетами. Но вот функции для нахождения катета по гипотенузе и катету нет.
...
Оффтоп: существуют реализации, где `operator%` используется для векторного произведения. Есть такие, где `operator^` используется для нахождения угла между векторами. Автор придерживается реализации через функции `r::cp(r)`, `r::dp(r)`, `r::ang(r)` для таких вещей, а `operator*` и `operator^` использует для нахождения точек пересечения и определения наличия пересечения каких-то геометрических объектов. На самом деле, читать потом такой код -- сущий ад, и желательно писать все нормально с самого начала.
