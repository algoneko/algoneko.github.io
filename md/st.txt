...
Оффтоп: вы не задумывались, почему мы перешли с 32-битных процессоров на 64-битные? Каждый указатель ссылается на байт — более точный адрес менеджер памяти выделять не умеет. Поэтому 32-битный компьютер умеет работать только с не более, чем $2^{32}$ байтами памяти — это ровно 4 гибибайта — что с какого-то момента начало нехватать. Большинство операций в любом компьютере — это операции с памятью, и разрядность повысили именно из-за этого, а не чтобы операции с `long long` быстрее считались.

Оффтоп-2: Похоже, изначальный автор конспекта на алгоритмике не слышал про [PAE](https://en.wikipedia.org/wiki/Physical_Address_Extension), с помощью которого можно адресовывать более 4 ГиБ памяти. Но на Windows этой штукой не пользуются -- она доступна только на серверных версиях и можеб быть несовместима с некоторыми драйверами. Использование PAE изменяет можель памяти, позволяя операционной системе иметь больше 4 ГиБ памяти, но каждая отдельная программа все равно ограничена использованием до 4 ГиБ памяти. Также, на современных 64-битных процессорах адресуется не ~2^64~ байт памяти, а [меньше](https://ru.wikipedia.org/wiki/64_%D0%B1%D0%B8%D1%82%D0%B0) (почти всегда ~2^48~).
...
**«ДО снизу»**. Можно делать все операции итеративно — так получится раз в 7 быстрее, но писать что-либо нетривиальное (например, массовые операции) так будет намного труднее. Подробнее смотрите в [посте с CodeForces](https://codeforces.com/blog/entry/18051), [перевод](st2.html).
