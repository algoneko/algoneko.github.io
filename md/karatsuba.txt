...
Историческое примечание: эта задача сейчас решается за $\O(n \log n)$ с помощью алгоритма [быстрого преобразования Фурье](fft.html), который к концу 50-х годов уже изобрели, но использовали только «по назначению»: для обработки сигналов, а не для умножения чисел.
...
![](img/divide-and-conquer.png =400x*)
...
* Все арифметические операции нужно реализовать как простые линейные проходы по массивам, чтобы компилятор смог их [векторизовать](sse.html).
...
```cpp
// Научись уже пользоваться FFT, Java или Python
...
Решение в условиях задачи работает за 3 секунды с ограничением в 5. Его можно значительно ускорить, если вместо `int`-ов использовать байты или даже биты по аналогии с [битсетом](bitset.html), потому что нас не интересует количество способов набрать какую-то сумму, а только равно ли это число нулю.
